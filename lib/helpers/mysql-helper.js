...
function makeVerificationBackup(transactionId, subscribeReq) {
    ...
    var query = 'insert into applicant_transaction_history ' +
        ...
        'identity_verification_attempts,social_media_data,verifications,activities) select * from applicant_transaction where id = ?';
    ...
    return command(query, params).then(function(result) {
        var id = undefined;
        if(_.isNil(result)) {
