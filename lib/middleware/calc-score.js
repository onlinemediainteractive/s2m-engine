...
function getSocalMediaData(applicantTransaction) {
    ...
    var params = [applicantTransaction.getApplicationId() , applicantTransaction.getApplicantRefId(), 'active'];
...
function scoreCanBeCalulated(applicantTransaction) {
    ...
    if((applicantTransaction.ssnTraceRequired()) && (onfidoConfig.active == true)) {
...
function getSsnTracePoints(applicantTransaction, scoreTrace) {

    var availablePoints = 20;

    scoreTrace.ssnTracePoints = 0
    scoreTrace.ssnTraceReason = "SNN Trace Successfull";

    if((applicantTransaction.ssnTraceRequired()) && (onfidoConfig.active == true)) {
        availablePoints = availablePoints - 20;
        scoreTrace.ssnTraceReason = "SNN Trace Failed";
    };

    scoreTrace.ssnTracePoints = availablePoints;
    //return availablePoints;
    return scoreTrace;

};

function getIdentityVarificationPoints(applicantTransaction, scoreTrace) {
    var availablePoints = 20;

    scoreTrace.idVerificationPoints = 0
    scoreTrace.idVerificationReason = "Identity Verification Successfull";

    if(applicantTransaction.quizRequired()) {
        availablePoints = availablePoints - 20;
        scoreTrace.idVerificationReason = "Identity Verification Failed";
    };

    //return availablePoints;
    scoreTrace.idVerificationPoints = availablePoints;
    return scoreTrace;
};

function getSexOffenderPoints(applicantTransaction) {

    var deductPoints = -20;
    if(applicantTransaction.getCriminalSource() == 'onfido') {
    ...
    if(applicantTransaction.getCriminalSource() == 'lexis-nexis') {
        var offenceCnt = applicantTransaction.getSexOffenderVerification().recordCount;
        logger.debug('Sex Offender Offence Count = ' + offenceCnt);
        if(offenceCnt == 0) {
            deductPoints = 0;
        }
...
function getNationalCriminalPoints(applicantTransaction) {
    ...
    if(applicantTransaction.getCriminalSource() == 'lexis-nexis') {
        ...
        var offenceCnt = applicantTransaction.getNationalCriminalVerification().recordCount;
        if(offenceCnt > 0) {
            deductPoints = -10;
        }
        else {
            logger.debug('National Criminal Offence Count = 0');
        };
...
function getCriminalPoints(applicantTransaction,scoreTrace) {
    var availablePoints = 20;

    scoreTrace.criminalPoints = 0;
    scoreTrace.criminalReason = "";
    if(applicantTransaction.sexOffenderVerificationRequired() || applicantTransaction.nationalCriminalVerificationRequired()) {
        ...
        var sexOffenderPoint = getSexOffenderPoints(applicantTransaction);
        if(sexOffenderPoint != 0) {
            availablePoints = availablePoints + sexOffenderPoint;
            scoreTrace.criminalReason = scoreTrace.criminalReason + "Has Sex Offence on Record ";
        }

        var nationalCriminalPoint =  getNationalCriminalPoints(applicantTransaction);
        if(nationalCriminalPoint != 0) {
            availablePoints = availablePoints + nationalCriminalPoint;
            scoreTrace.criminalReason = scoreTrace.criminalReason + "Has Criminal Offence on Record";
        }
    ...
    scoreTrace.criminalPoints = availablePoints;
    //return availablePoints;
    return scoreTrace;
};

//
//function getCriminalBackGroundPoints(applicantTransaction) {
//    var availablePoints = 20;
//    return availablePoints;
//}
...
module.exports = function (req, res, next) {
    ...
    if(!canCalcScore.scoreCanBeCalulated) {
        ...
        scoreTrace =  getSsnTracePoints(applicantTransaction, scoreTrace);
        stepPoints = scoreTrace.ssnTracePoints;
        ...
        scoreTrace = getIdentityVarificationPoints(applicantTransaction,scoreTrace );
        stepPoints = scoreTrace.idVerificationPoints;
        score      = score + stepPoints;
        ...
        //logger.debug(' -- ' + 'Identity Varification - Step(3) Points = ' + stepPoints + ' Running Score = ' + score);
         ...
         scoreTrace = getCriminalPoints(applicantTransaction,scoreTrace);
         stepPoints  = scoreTrace.criminalPoints
         score      = score + stepPoints;
         logger.debug(' -- ' + 'Criminal Verification - Step(3) Points = ' + stepPoints + ' Running Score = ' + score);
         scoreTrace.criminalPoints = (stepPoints * 10);
