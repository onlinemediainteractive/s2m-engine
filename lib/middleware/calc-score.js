...
function getSocialMediaPoints(applicantTransaction) {
    ...
    logger.debug('*** full app trans : ' + JSON.stringify(applicantTransaction));
    logger.debug('---');
    logger.debug('*** applicantCreation : ' + JSON.stringify(applicantTransaction.applicantCreation));
    logger.debug('---');
    logger.debug('*** getSSN : ' + applicantTransaction.getSsn());
    logger.debug('** get params :' + JSON.stringify(applicantTransaction.getParams()));
    logger.debug('*** getLastName : ' + applicantTransaction.getLastName());
    return using(
        ...
        logger.debug("1st applicantTransaction :" + JSON.stringify(applicantTransaction));
            ...
            _.forEach(result, function(value) {
                ...
                if(value.status == 'active') {
                    if(value.source == 'facebook') {
                        ...
                        logger.debug("**** middle facebook name check ");
                        ...
                        logger.debug("**** end of facebook read socialmedia");
        ...
        logger.debug("2nd applicantTransaction :" + JSON.stringify(applicantTransaction));

        // This section same for all ages
        try {
            logger.debug("**** strting facebook name check after try ");
            if(facebook == 1)
            {
                logger.debug("**** strting facebook name check ");
                var fmfnMatch = false;
                var fmlnMatch = false;

                //if (scoreTrace.facebookFullName  == applicantTransaction.getFirstName()) {
                //    fmfnMatch = true;
                //}
                logger.debug("applicantTransaction.params :" + JSON.stringify(applicantTransaction.params));
                logger.debug("**** before first name check ");
                logger.debug("**** before first name check scoreTrace.facebookFirstName - " + scoreTrace.facebookFirstName);
                logger.debug("**** before first name check  applicantTransaction.getFirstName- " + applicantTransaction.getFirstName());
                logger.debug("applicantTransaction :" + JSON.stringify(applicantTransaction));
                logger.debug('facebook  first name check - ' +  scoreTrace.facebookFirstName + ' = ' +  applicantTransaction.getFirstName());

                if (scoreTrace.facebookFirstName == applicantTransaction.getFirstName()) {
                    fmfnMatch = true;
                }
                logger.debug("**** after first name check ");

                logger.debug("**** before last name check ");
                logger.debug('facebook last name check - ' +  scoreTrace.facebookLastName + ' = ' +  applicantTransaction.getLastName());
                if (scoreTrace.facebookLastName == applicantTransaction.getLastName()) {
                    fmlnMatch = true;
                }
                logger.debug("**** after last name check ");
            ...
            if(socialAccounts == 3) {
                ...
                if(socialAccounts == 2) {
                    if(facebook  == 1 && twitter == 1) {
                        availablePoints = availablePoints + 10;
                        scoreTrace.facebookAccountPoints = 10;
                        scoreTrace.facebookAccountPointsReason = "age <= 24 and  2 social media accounts facebook and twitter";
                    }
                    if(facebook  == 1 && linkedin == 1) {
                        availablePoints = availablePoints + 10;
                        scoreTrace.facebookAccountPoints = 10;
                        scoreTrace.facebookAccountPointsReason = "age <= 24 and 2 social media accounts facebook and linkedin";
                    }
                }
            }
            if(friends < 300) {
            ...
            if(socialAccounts == 3) {
                ...
                if(socialAccounts == 2) {
                    if(facebook  == 1 && twitter == 1) {
                        availablePoints = availablePoints + 5;
                        scoreTrace.facebookAccountPoints = 5;
                        scoreTrace.facebookAccountPointsReason = "> 24 age <= 44 and  2 social media accounts facebook and twitter";
                    }
                    if(facebook  == 1 && linkedin == 1) {
                        availablePoints = availablePoints + 8;
                        scoreTrace.facebookAccountPoints = 0;
                        scoreTrace.facebookAccountPointsReason = "> 24 age <= 44 and  2 social media accounts facebook and linkedin";
                    }
                }
            }
            if(friends < 300) {
            ...
            if(socialAccounts == 3) {
                ...
                if(socialAccounts == 2) {
                    if(facebook  == 1 && twitter == 1) {
                        availablePoints = availablePoints + 5;
                        scoreTrace.facebookAccountPoints = 5;
                        scoreTrace.facebookAccountPointsReason = "> 44 age <= 54 and 2 social media accounts facebook and twitter";
                    }
                    if(facebook  == 1 && linkedin == 1) {
                        availablePoints = availablePoints + 8;
                        scoreTrace.facebookAccountPoints = 8;
                        scoreTrace.facebookAccountPointsReason = "> 44 age <= 54 and 2 social media accounts facebook and linkedin";
                    }
                }
            }
            if(friends < 300) {
        ...
        if(age > 54 && age <= 64) {
           ...
           //     scoreTrace.friendPointsReason = "> 54 age <= 64 and 0 social media accounts";
            ...
            if(socialAccounts == 3) {
                availablePoints = availablePoints + 10;
                scoreTrace.friendPoints = 5;
                scoreTrace.friendPointsReason = "> 54 age <= 64 and 3 social media accounts";
            }
            else {
                if(socialAccounts == 2) {
                    if(facebook  == 1 && twitter == 1) {
                        availablePoints = availablePoints + 5;
                        scoreTrace.facebookAccountPoints = 5;
                        scoreTrace.facebookAccountPointsReason = "> 54 age <= 64 and 2 social media accounts facebook and twitter";
                    }
                    if(facebook  == 1 && linkedin == 1) {
                        availablePoints = availablePoints + 8;
                        scoreTrace.facebookAccountPoints = 8;
                        scoreTrace.facebookAccountPointsReason = "> 54 age <= 64 and 2 social media accounts facebook and linkedin";
                    }
                }
            }
            if(friends < 300) {
        ...
        if(age > 64) {
            ...
            if(socialAccounts == 0) {
                ...
                scoreTrace.friendPoints = 7;
                scoreTrace.friendPointsReason = "age > 64 and 0 social media accounts";
            }
            if(socialAccounts == 3) {
                availablePoints = availablePoints + 10;
                scoreTrace.friendPoints = 10;
                scoreTrace.friendPointsReason = "age > 64 and 3 social media accounts";
            }
            else {
                if(socialAccounts == 2) {
                    if(facebook  == 1 && twitter == 1) {
