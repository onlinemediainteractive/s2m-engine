    ...
    if(_.isNil(socialMedia)) {
        ...
        _.forEach(socialMedia, function(value, key) {
           ...
           // logger.debug("attributes : " + value);
            ...
            if((value.accessToken !== 'N') && (value.userId !== 'N') && (value.accessToken !== '') && (!_.isNull(value.accessToken))) {
                req.socialMediaData.push({"source" : key, "attributes" : value});
            }
            else {
                logger.debug('Social Media Type ' + key + ' exclude  accessToken = ' + value.accessToken);
            }
