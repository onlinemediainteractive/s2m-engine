...
function inactivateSocalMedia(applicantRefId, sourceList, status) {
...
module.exports = function(req, res, next) {
    ...
    if(_.isNil(socialMedia)) {
        ...
        _.forEach(socialMedia, function(value, key) {
            ...
            if(key == 'facebook') {
               if(value.accessToken !== '') {
                   ...
                   sourcelistOn.push({"source" : key})
               } else {
                   sourcelistOff.push({"source" : key})
               }
            }
            else {
                if(value == 'Y') {
        ...
        inactivateSocalMedia(req.applicantTransaction.getApplicantRefId(), sourcelistOff, 'inactive').then(function (resultId) {
            //    logger.debug('Social media rows updated : ' + resultId.rowsChanged);
            inactivateSocalMedia(req.applicantTransaction.getApplicantRefId(), sourcelistOn, 'active').then(function (resultId) {
                //    logger.debug('Social media rows updated : ' + resultId.rowsChanged);
                logger.debug(scriptName + ' Continue ....');
                next();
            });
        ...
        /*if (sourcelist.length > 0) {
            inactivateSocalMedia(req.applicantTransaction.getApplicantRefId(), sourcelistOff).then(function (resultId) {
                //    logger.debug('Social media rows updated : ' + resultId.rowsChanged);
                logger.debug(scriptName + ' Continue ....');
                next();
            });
