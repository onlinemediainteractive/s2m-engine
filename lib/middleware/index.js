module.exports = {
    apiGetApplication: require('./get-application')
   ,apiGetApplicant: require('./get-applicant-transaction')
   ,apiContinueProcess : require('./continue-process')
   ,apiLogRequest: require('./log-request')
   ,apiCreateApplicant : require('./create-applicant')
   ,apiSsnTrace : require('./ssn-trace')
   ,apiVerifyIdentity : require('./verify-identity')
   ,apiNationalCriminalVerificationLN : require('./national-criminal-verification-ln')
   ,apiNationalCriminalVerification : require('./national-criminal-verification')
   ,apiSexOffenderVerificationLN : require('./sex-offender-verification-ln')
   ,apiSexOffenderVerification : require('./sex-offender-verification')
   ,apiCalcScore : require('./calc-score')
   ,apiGetScoreMock : require('./get-score-mock')
   ,apiApplicantState : require('./validate-applicant-state')
   ,apiSocialMediaVerification : require('./social-media-verification')
   ,apiParseSocialMedia : require('./parse-social-media')
   ,apiFacebookExtendToken : require('./facebook-extend-token')
   ,apiTwitterExtendToken : require('./twitter-extend-token')
   ,apiLinkedinExtendToken : require('./linkedin-extend-token')
   ,apiUpdateApplicant : require('./update-applicant')
   ,apiIdentityVerificationComplete : require('./identity-verification-complete')
   ,apiRefreshLocalToken : require('./refresh-social-token')
   //,apiAuthentication : ('./api-authentication')
};
