                ...
                getExtendedToken(extendedToken).then(function(socialHttpResponse) {
                    ...
                    //if(!_.isNull(socialHttpResponse.s2mResponse)) {
                      ...
                      //  var socialHttpResponseJson = JSON.parse('{"' + socialHttpResponse.httpResponse + '"}');
                        ...
                        FB.api('me', { fields: ['id', 'name', 'email', 'first_name', 'last_name', 'birthday', 'age_range', 'gender', 'link',
                                               'is_verified','verified','friends','family','significant_other','relationship_status', 'work'] }, function (facebookProfile) {
                            ...
                            if(_.isEmpty(facebookProfile.data)) {
                                logger.error("Empty Facebook profile graph returned for Applicant Id : " + applicantTransaction.getApplicantRefId());
                                logger.debug(scriptName + ' Continue ....');
                                next();
                            }
                            else {
