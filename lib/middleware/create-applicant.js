...
function insetApplicantTransaction(req, applicantCreationJson, status) {
    ...
    if(onfidoConfig.active == false)
       query = query + " , ssn_verification, ssn_verification_status) " +
          ' values(?, ?, ?, ?, ?, ?, ?, ?)';
    else {
        ') values(?, ?, ?, ?, ?, ?)';
    }
   ...
   // params.push(encrypt.encryptDbString(JSON.stringify(applicantCreationJson)));
    ...
    if(onfidoConfig.active == false) {
        ...
        ssnTraceMsg = {'message' : ' No ssn trace active setting to success and moving on'};

        params.push(JSON.stringify(ssnTraceMsg));
        params.push('success');
    }
   ...
   // applicantTransaction = undefined;
    ...
    if(req.applicantTransaction.ssnTraceRequired()) {
        ...
        //onfido.createApplicant(req).then(function (response) {
            ...
            applicantCreationJson.serviceResponse = {"message " : "Creating Applicant Transaction Only"};
